	...
	private void startNieuwSpel() {
		...
		do {
			try {
				do {
					System.out.printf(
							"Wilt u nog een speler toevoegen?%n1. Voeg speler toe%n2. Start het spel%nJouw keuze > ");
					...
					if (keuzeNieuw != 1 && keuzeNieuw != 2) {
						...
						if (keuzeNieuw == 1) {
							...
							alleBeschikbareKleuren = dc.geefBeschikbareSpelerKleuren(); // dit is een lijst van
																						// SpelerKleur
						}
		...
		do {
			try {
				...
				for (int index = 1; index <= lijstVanSpelers.size(); index++) {
					System.out.printf("%d. \t %-20s met als geboortejaar %d%n", index,
							lijstVanSpelers.get(index - 1).gebruikersnaam(),
							lijstVanSpelers.get(index - 1).geboortejaar()); // Alleen gebruikersnaam genoeg?
				}
